	...
	print (style) {
		...
		this.sharedFunctions.forEach(funcConf => {
			if (postProcessing.isPostProcessor(funcConf.func)) {
				...
				queries = queries.map(kq => kq.inheritMethod(funcConf.func, funcConf.params));
			}
		});
		style = style || this._printer;

		if (style === 'qs' || style === 'qo' || style === 'url') {
			return Promise.all(queries.map(q => q.print(style)));
		}
		...
		return Promise.all(queries.map(q => q.print('json')))
