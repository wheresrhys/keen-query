...
function concat (tables, primaryDimension) {
	...
	if (firstTable.dimension === 0 && otherTables.every(t => t.dimension === 0)) {
		...
		// if (primaryDimension) {
		// 	tables = tables.map(t => t.dimension > 1 ? t.switchDimensions(null, dimension, 'shuffle') : t);
		// }
		const table = firstTable.clone();
		...
		newHeadings.forEach((name, i) => {
			table.axes[0].values.forEach((v, j) => {
				table.data[`${j},${table.axes[1].values.length + i}`] = otherTables[i].data[j];
			})
		});
