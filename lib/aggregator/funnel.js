...
class Funnel {
	...
	print (style) {
		...
		return Promise.all(this.queries.map(q => q.print('funnelStep')))
			.then(queries => {
				const base = config.KEEN_HOST || 'https://api.keen.io/3.0';
				const url = `${base}/projects/${config.KEEN_PROJECT_ID}/queries/funnel?api_key=${config.KEEN_READ_KEY}&steps=${encodeURIComponent(JSON.stringify(queries))}`;
				if (style === 'url') {
					return url;
				}
				return fetch(url)
	...
	toString () {
		...
		let str = `@funnel(${this.queries.map(q => q.toString()).join(',')})`;
		const actorProperty = this.getActorProperty();
		if (actorProperty) {
			str += `->with(${actorProperty})`;
		}
		return str;
	}

	getActorProperty () {
		const withFunc = this.sharedFunctions.find(func => func.func === 'with')
		return withFunc && withFunc.params[0];
	}
	...
	get valueLabel () {
		const actorProperty = this.getActorProperty();
		return actorProperty ? `count(${actorProperty})` : 'count';
	}
}
