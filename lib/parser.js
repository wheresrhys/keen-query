...
function validateAbsTime(conf) {
	if (conf.params.length < 2) {
		throw 'Must specify both a start and end date when using ->absTime()'
	}
...
function validateFunction (conf) {
	...
	if (['filter', 'group'].indexOf(name) > -1) {
		return isValidStructure && validateSimple(conf);
	}

	if (name === 'reduce') {
		return isValidStructure && validateSimple(conf) && validateReduce(conf);
	}

	if (name === 'relTime') {
		return isValidStructure && validateSimple(conf) && validateRelTime(conf);
	}

	if (name === 'absTime') {
		return isValidStructure && validateAbsTime(conf);
	}

	if (name === 'interval') {
		return isValidStructure && validateSimple(conf) && validateInterval(conf);
	}
