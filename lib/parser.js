...
class Deconstructor {
	...
	deconstructQuery (str) {
		...
		return {
			event: eventName,
			functions: this.deconstruct(str)
		}
	}
...
logO(deconstruct('page:view->count(apples)->filter(\'pickle\')'))
logO(deconstruct('@ratio(page:view->count(apples),page:view->filter(\'pickle\'))->group(lobs)'))
