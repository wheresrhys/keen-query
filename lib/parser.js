...
const extractionNames = ['min', 'max', 'sum', 'avg', 'med', 'count', 'select', 'median', 'pct', 'percentile'];
...
function validateFunction (conf) {
	...
	if (name === 'sortAsc' || name === 'sortDesc' || name === 'plotThreshold'|| name === 'cutoff'|| name === 'top'|| name === 'bottom') {
	...
	deconstruct (str) {
		...
		for (let i = 0, il = str.length; i < il; i++) {
			...
			if (openParantheses && !func) {
				...
				if (postProcessing.isPostProcessor(func)) {
