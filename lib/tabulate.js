...
function crossTable (name, data) {
	...
	table.headings = [this.secondaryGrouping].concat(cols);
	if (this.secondaryGrouping) {
		table.rows = rows.map((k, i) => {
			const row = [k];
			cols.forEach(name => {
				data.result.some(res => {
					if (res[this.query.group_by] === name) {
						const transformedK = utils.transformValue(k)
						const val = res.value.find(v => v[this.secondaryGrouping] === transformedK);
						if (val) {
