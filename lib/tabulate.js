	...
	if (this.groupedBy) {
		if (this.groupedBy.length > 1 ) {
			if (this.groupedBy.length > 2 || this.query.interval) {
				...
				return crossTable.call(this, prepDoubleGroupedData(data, this.groupedBy));
			}
		}
		if (this.query.interval) {
			data = crossTable.call(this, prepIntervalGroupedData(data, this.intervalUnit, this.groupedBy[0]));
			return this.flipTable ? flip(data) : data;
		}
