...
function simpleTable (curr, prev) {
	...
	table.headings = ['', this.timeframe];
	if (prev) {
		table.headings.push(this.prevTimeframe);
	}
	...
	if (this.groupedBy) {
		if (this.groupedBy.length > 1 ) {
			if (this.groupedBy.length > 2 || this.query.interval) {
				...
				const results = [crossTable.call(this, this.timeframe + ' ' + this.groupedBy[1], prepDoubleGroupedData(curr, this.groupedBy))];
				if (prev) {
					results.push(crossTable.call(this, this.prevTimeframe + ' ' + this.groupedBy[1], prepDoubleGroupedData(prev, this.groupedBy)));
				}
				return results;
			}
		}
		if (this.query.interval) {
			const results = [crossTable.call(this, this.timeframe, prepIntervalGroupedData(curr, this.intervalUnit, this.groupedBy[0]))];
			if (prev) {
				results.push(crossTable.call(this, this.prevTimeframe, prepIntervalGroupedData(prev, this.intervalUnit, this.groupedBy[0])));
			}
			return results;
		}
