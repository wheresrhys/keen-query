...
class Table {
	constructor (conf) {
		...
		this.eventCollection = conf.eventCollection;
		this.targetProperty = conf.targetProperty;
	}
	...
	get valueLabels () {
		if (!this.targetProperty) {
			return `${this.metric} ${this.eventCollection}`;
		} else {
			return `${this.eventCollection} (${this.metric.replace(/_/g, ' ')} ${this.targetProperty})`;
		}
	}
	...
	humanize (timeFormat) {
		...
		if (this.dimension === 0) {
			...
			humanized.rows = [[table.valueLabels, table.data]]
			return humanized;
		} else if (table.dimension === 1) {
			humanized.headings = [table.axes[0].property, table.valueLabels]
	...
	switchDimensions (a, b, method) {
	  ...
	  if (a > this.dimensions || b > this.dimensions) {
			throw `This table does not have a ${Math.max(a, b)} dimension`;
		}
