...
class Table {
	...
	// finds what the coords of the same point would be for a table wit hthose axes
	...
	findEquivalentCoords (coords, alternateAxes) {
		...
		coords = coords.map((n, i) => {
			const referenceValue = this.axes[i].values[n];
			if (typeof referenceValue === 'string') {
				return alternateAxes[i].values.indexOf(this.axes[i].values[n]);
			}
			return alternateAxes[i].values.indexOf(alternateAxes[i].values.find(dateObj => {
				return dateObj.start === this.axes[i].values[n].start;
			}));
		});
...
const postProcessors = require('./mappings').postProcessors;

module.exports.mixin = function (wrapperPrototype) {
	postProcessors.forEach(method => {
