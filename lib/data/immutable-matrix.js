...
class ImmutableMatrix {
	...
	unflatten (timeFormat) {
		...
		const matrix = this.convertTime(timeFormat);
		const output = {};
		if (this.dimension === 0) {
			// just send back a single cell
			output.rows = [[matrix.valueLabel, matrix.data]]
			return output;
		} else if (matrix.dimension === 1) {
			// if (matrix.axes[0].property === '_headings') {
			// 	output.headings = matrix.axes[0].values;
			// } else {
				output.headings = [matrix.axes[0].property, matrix.valueLabel]
			// }
			output.rows = matrix.axes[0].values.map((label, i) => {
				return [label, matrix.data[i]]; // this works because [i] notation works on an object or an array :-)
			})
		} else {
			const data = matrix.toNested();

			output.headings = [matrix.axes[0].property].concat(matrix.axes[1].values);
			output.rows = matrix.axes[0].values.map((label, i) => {
