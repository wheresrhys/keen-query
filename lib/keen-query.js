...
class KeenQuery {
	...
	static execute (str) {
		console.log(str);
		if (/^@compare/.test(str)) {
			return new Compare(str).print();
		} else if (/^@ratio/.test(str)) {
			return new Ratio(str).print();
		} else if (/^@concat/.test(str)) {
			const concatConf = /^@concat\((.*(?:\!\.\..*)+)\)(.*)?$/.exec(str);
			return new Concat(concatConf[1].split('!..'), concatConf[2])
		}
